...
mongoose.connect("mongodb+srv://oussamarhouch:<Oussama2002.>@cluster1.dp3aos9.mongodb.net/?retryWrites=true&w=majority", {useNewUrlParser: true});
...
app.get("/operations", function(req, res){
    if(req.isAuthenticated()){
        Operations.find({"username": req.user.username}, function(err, foundOperations){
            if(err){
                console.log(err);
            } else{
                res.render("operations", {newListOperations: foundOperations, total: 0, statut: 1});
            }
        });               
    } else{
        res.redirect("/login");
...
app.post("/register", function(req, res){
    User.register({ username: req.body.username, email: req.body.email, LastName: req.body.LastName, FirstName: req.body.FirstName}, req.body.password, function(err, user){
        if(err){
            console.log(err);
